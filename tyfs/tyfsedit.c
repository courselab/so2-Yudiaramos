...
/* Automatic format function */
void auto_format(const char *volume_path, int boot_sectors, int max_file_size_kb);
...
int main(int argc, char *argv[])
{
  ...
  /* Check for automatic format mode */
  if (argc == 5 && strcmp(argv[1], "-f") == 0) {
    auto_format(argv[2], atoi(argv[3]), atoi(argv[4]));
    return 0;
  }
  ...
  while (go_on)
  {
    ...
    i = 0;
    while ((arg_values[i] = strtok(i == 0 ? buffer : NULL, " \t\n"))) i++;
    arg_count = i;
    ...
    if (arg_values[0])
      while (cmds[i].func)
      {
        if (!strcmp(arg_values[0], cmds[i].name))
        {
          cmds[i].func(arg_count, (const char**)arg_values);
          break;
        }
        i++;
      }
...
/* Function to handle automatic formatting */
void auto_format(const char *volume_path, int boot_sectors, int max_file_size_kb) {
  int i, rs;

  volume_fp = fopen(volume_path, "r+");
  if (!volume_fp) {
  ...
  if (rs < 0) {
    fprintf(stderr, "Error seeking in volume\n");
    exit(1);
  }
  ...
  fs_header.number_of_boot_sectors = boot_sectors;
  fs_header.max_file_size = max_file_size_kb * 2;

  /* Compute how may files the volume can support. */

  float number_of_entries = (float)
